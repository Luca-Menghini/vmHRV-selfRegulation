...
```{r }
mvn(data=ema[,mood],univariatePlot="histogram")[4] # histogram mood
mvn(data=ema[,mood],univariatePlot="qqplot")[4] # qqnorm mood
```
...
First, we prepare the data to be used in the analysis. That is, we select the dataset of complete responses to any mood item, which we call `Mood`.
```{r }
...
round(fit.ind(m.withinPool_Mood,type="monolevel"),3) # fit indices
```
...
plot.traj(ema,var="HT","HRV068") # checking participant HRV068
```
...
Finally, we evaluate the fit of these benchmark models. If the structure on level 2 holds, all models (and especially the Null and the Independence model) should be rejected. Accordingly, we can note that both the null and the independence models are rejected, whereas acceptable fit is shown by the **saturated model**. Consequently, the saturated model will be considered for evaluating the fit of the hypothesized models.
```{r message=TRUE,warning=TRUE}
round(fit.ind(model=c(m.null_Mood,m.ind_Mood,m.sat_Mood),
              models.names=c("Lv2 Null","Lv2 Independence","Lv2 Saturated")),3)
```

<br>

#### 2.1.2.3. Multilevel models
...
As a strategy to solve the improper solutions, we refit the models by **fixating the residual variance** of problematic items to the 15% of the variance estimated for these items at level 2. In other words, we make the assumption that these items have a reliability around 85% (see [Joreskog & Sobrom, 1996](#ref)).
```{r message=TRUE,warning=TRUE}
# m3x3: fixing residual variance of item t2
m3x3.fix <- 'level: 1
             hedTone_w =~ v1 + v2 + v3
             Calmness_w =~ t1 + t2 + t3
             enArousal_w =~ f1 + f2 + f3
...
# m2x3: fixing residual variance of item t2 
m2x3.fix <- 'level: 1
             hedTone_w =~ v1 + v2 + v3
             Calmness_w =~ t1 + t2 + t3
             enArousal_w =~ f1 + f2 + f3
             ...
             enArousal_b =~ f1 + f2 + f3
             t2 ~~ rho2 * t2'
m2x3.fix <- cfa(gsub("rho2",t2varlv2*.15,m2x3.fix),data=Mood,cluster='ID',std.lv=TRUE,missing="fiml") # fixating rho2


# ARRIVATO QUI - IN REALTà NON SI RISOLVE UN CAZZO
```
