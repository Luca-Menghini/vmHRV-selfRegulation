...
```{r }
# required packages
packages <- c("ggplot2","gridExtra","jsonlite","plyr","lubridate","Rmisc")
...
readSurveyData <- function(data.path,probe.definition,messages=TRUE){ require(jsonlite)
  ...
  for(double in doubles){ subId <- levels(as.factor(data[data$id.t==double,"SubmissionTimestamp"])) # submission time
    if(length(subId)>1){ ndoubles[1] <- ndoubles[1] - 1
      ...
      data[data$id.t==double,"RunTimestamp"] <- maxDouble - 55.92 # 2nd RunTimeStamp = Submission - 55.92 sec (median resp time) 
  ...
  cat(" - Removed",ndoubles[1],"double responses (same ID, InputName, RunTimestamp, & SubmissionTimestamp)\n",
      ...
      "  (in these cases, RunTimestamp is recomputed as SubmissionTimestamp - 55.92 sec (average response length)")
  ...
  for(survey in levels(data$survey)){ data[data$survey==survey,"RT.tot"] <-
    difftime(data[data$survey==survey,"Timestamp"][nrow(data[data$survey==survey,])],
...
```{r warning=FALSE,message=FALSE}
library(lubridate); library(scales); library(plyr)
```
...
# correcting wrongly encoded RunTimestamps (adding 1year) and SubmissionTimestamps (RunTimestamp + 55.92 sec)
ESM[substr(ESM$RunTimestamp,1,7)=="2018-01","SubmissionTimestamp"] <-
  ESM[substr(ESM$RunTimestamp,1,7)=="2018-01","RunTimestamp"] + 55.92
...
# 2 further cases with missing SubmissionTimestamp (replacing with RunTimestamp + 55.92 sec)
ESM[is.na(ESM$SubmissionTimestamp),c("ID","RunTimestamp","SubmissionTimestamp")]
ESM[is.na(ESM$SubmissionTimestamp),"SubmissionTimestamp"] <- ESM[is.na(ESM$SubmissionTimestamp),"RunTimestamp"] + 55.92
```
...
We do not see any reason to exclude participants based on response rate criteria. However, as a potential covariate to be considered as a control, we arbitrarily choose to **flag participants with an overall response rate < 80%** (N = 9 among the included participants).
```{r }
# flagginng cases with response rate < 80%
rrate$respRate80 <- 0
rrate[rrate$ord.RR<80,"respRate80"] <- 1
ESM <- join(ESM,rrate[,c("ID","ord.RR","respRate80")],by="ID",type="left")
ESM$respRate80 <- as.factor(ESM$respRate80)
summary(ESM[!duplicated(ESM$ID) & substr(ESM$ID,1,3)=="HRV","respRate80"])

# showing flagged cases
rrate[rrate$respRate80==1,]
```

<br>

In addition to counting the number of received surveys, we must also consider **missing responses within each survey**. Indeed, a number of surveys was incomplete **due to technical problems** with the app. We can notice that missing responses **mainly concern items on confounder variables** (i.e., activity, alcohol, coffee, smoke and people), which were administered in the last part of each survey and showed from 49 to 58 missing values (about 4%). In contrast, missing data are only about the 2-3% for items measuring stressful events (from 26 to 35), and less than 2% for itmes measuring mood (from 0 to 2).
```{r }
...
Here, we flag participants and/or single responses that might be considered as **careless respondents/responses**. Following [Curran (2016)](#ref), we consider multiple indicators of potentially careless responses including response times, ...

##### 2.3.4.2.1. Response times
...
ESM.RT[substr(ESM.RT$RunTimestamp,1,7)=="2018-01","SubmissionTimestamp"] <-
  ESM.RT[substr(ESM.RT$RunTimestamp,1,7)=="2018-01","RunTimestamp"] + 55.92
...
We can see that **responding to a single item required about 2 seconds** in most cases, regardless of the type of item. A small number of items were responded in more than 10 seconds (4.3%) with a very few cases requiring more than 25 seconds (N = 157, 0.69%). Likewise, a small number of items were responded in less than 1 second (2.35%) with a very few cases requiring less than 0.65 seconds (N = 109, 0.48%).
...
# slow response times (N = 34)
ESM.RT[ESM.RT$iRT.slow==1,c("ID","RT.tot","NiRT.slow",
                            colnames(ESM.RT)[which(colnames(ESM.RT)=="v1"):which(colnames(ESM.RT)=="people")])]
...
ESM.RT[ESM.RT$ID %in% names(s[s >= 2]),"slowResp"] <- 1 # flagging slow respondents (i.e., 2+ responses with 1+ items > 50 sec)

# fast response times (N = 34)
ESM.RT[ESM.RT$iRT.fast==1,c("ID","RT.tot","NiRT.fast",
                            colnames(ESM.RT)[which(colnames(ESM.RT)=="v1"):which(colnames(ESM.RT)=="people")])]
summary(as.factor(ESM.RT$NiRT.fast)) # summary of number of fast response times
(s <- summary(as.factor(as.character(ESM.RT[ESM.RT$iRT.fast==1,"ID"])))) # summary of slow respondents
ESM.RT[ESM.RT$ID %in% names(s[s >= 3]),"fastResp"] <- 1 # flagging fast respondents (i.e., 3+ responses with 1+ items > 50 sec)
```

<br>

Then, the same procedures are applied by considering the **total response time of each survey** `RT.tot`, computed in section 2.1 as the time between the first and the last response of each survey, here expressed in minutes. Note that for fast total response times we do not consider those cases with missing response to the last item (i.e., from `PE` above).

We can see that **responding to the whole questionnaire required about 55 sec** in most cases (median = 55.92 sec). A small number of questionnaires were responded in more than 2 minutes (5.65%) with a very few cases requiring more than 5 min (N = 12, 0.88%, flagged as `tRT.slow = 1`). Likewise, a small number of questionnaires were responded in less than 40 second (14.79%) with a very few cases requiring less than 35 seconds (N = 84, 6.21%, flagged as `iRT.fast = 1`). We also flag **careless participants** as those with 2+ responses that took more than 5 min (flagged as `slowResp.tot = 1`) and 3+ responses that took less than 35 seconds (flagged as `fastResp.tot = 1`).
...
# flagging slow and fast total response times
ESM.RT$tRT.slow <- ESM.RT$tRT.fast <- ESM.RT$slowResp.tot <- ESM.RT$fastResp.tot <- 0
ESM.RT[ESM.RT$RT.tot.min > 5,"tRT.slow"] <- 1
...
ESM.RT[ESM.RT$ID %in% names(s[s >= 2]),"slowResp.tot"] <- 1 # flagging slow respondents (i.e., 2+ responses > 5 min)
(s <- summary(as.factor(as.character(ESM.RT[ESM.RT$tRT.fast==1,"ID"])))) # summary of fast respondents
ESM.RT[ESM.RT$ID %in% names(s[s >= 3]),"fastResp.tot"] <- 1 # flagging fast respondents (i.e., 3+ responses < 35 sec)
```


<br>

##### 2.3.4.2.2. Consistency
...
ratings <- c("v1","v2","v3","t1","t2","t3","f1","f2","f3")

ESM$eo <- evenodd(ESM[,ratings],factors=rep(3,3))
hist(ESM$eo)

knitr::kable(ESM[!is.na(ESM$eo) & ESM$eo>0.5,c("ID",ratings)])
```
